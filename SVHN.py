...
class Draw():
    ...
    def train(self):
        data = glob("Datasets/SVHN/png_train/*.png")
        ...
        np.save("genloss_history_svhn_attn.npy", np.array(self.genloss_history))
        np.save("latloss_history_svhn_attn.npy", np.array(self.latloss_history))

        # tf.keras.models.save_model(self.sess, 'your_model_name.h5')

    def view(self):
        data = glob("Datasets/SVHN/png_test/*.png")
        base = np.array([get_image(sample_file, 108, is_crop=True) for sample_file in data[0:64]])
        base += 1
        base /= 2
    ...
    def generate(self):
        data = glob("Datasets/SVHN/png_test/*.png")
        random_index_from = np.random.randint(0, len(data) - 64)
        index_to = random_index_from + 64
        sample_images = np.array([get_image(sample_file, 108, is_crop=True) for sample_file in data[random_index_from:index_to]])

        saver = tf.train.Saver(max_to_keep=2)
        saver.restore(self.sess, tf.train.latest_checkpoint(os.getcwd() + "/training/"))

        cs, attn_params = self.sess.run([self.cs, self.attn_params], feed_dict={self.images: sample_images})
        cs = 1.0 / (1.0 + np.exp(-np.array(cs)))
        results = cs[-1]
        results_square = np.reshape(results, [-1, self.img_size, self.img_size, self.num_colors])
        title = "generated/{:%Y-%m-%d_%H-%M-%S}-generate.jpg".format(datetime.now())
        ims(title, merge_color(results_square, [8, 8]))
